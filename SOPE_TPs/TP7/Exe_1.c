...
a) Each thread will increment by one the number of "Numbers" put. At the end the sum has to reach
the value specified by the parameter.

b) The buffer, pos and value are moved by each thread at the same time. If one of the instructions gets interrupted in the middle,
to execute another thread, then the output will be incorrect, resulting in lots of errors.z
